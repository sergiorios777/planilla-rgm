...
func registrarRutasTenant(mux *http.ServeMux, db *sql.DB) {
	...
	mux.HandleFunc("/tenant/plame/trabajadores-padron", middleware.RequireAuth(plameHandler.VistaPadronTrabajadores))
	mux.HandleFunc("/tenant/plame/conceptos-nomina", middleware.RequireAuth(plameHandler.VistaConceptosNomina))
	mux.HandleFunc("/tenant/plame/conceptos-nomina/reasignar", middleware.RequireAuth(plameHandler.ReasignarConceptoNominaHTMX))
