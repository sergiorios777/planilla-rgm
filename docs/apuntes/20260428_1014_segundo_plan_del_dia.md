...
* **Librería a usar**: Integraremos [github.com/xuri/excelize/v2](https://github.com/xuri/excelize/v2), que es el estándar de oro en Go para leer y escribir archivos .xlsx de forma nativa y ultra rápida.
...
* **Paso 2 (Repositorio y Reglas de Negocio)**: Crearemos la función `CambiarEstado(id, estado)` en el repositorio. Además, modificaremos tu función `Procesar` en el `planillaService` para inyectar una regla de oro: _Si la planilla está `CERRADA`, el motor de cálculo aborta inmediatamente devolviendo un error_.
